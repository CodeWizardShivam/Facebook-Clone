<script lang="ts">
	// import img from '../../../static/facebook_logo.svg';
	import '../app.postcss';
</script>
...
<html lang="en" class="bg-gray-200">
	<head>
		...
		<!-- <link rel="icon" type="image/png" href="./favicon.png"><link rel="icon" type="image/png" href="https://www.facebook.com/favicon.ico" /> -->
	</head>
	<div class="flex">
		<div class="left flex flex-col justify-stretch">
			<svg
				class="scale-[.3] -mx-60"
			...
			<span class="mx-44 text-3xl -my-32"
				>Facebook helps you connect and share <br /> the people in your life.</span
			>
		</div>

		<div
			class="right bg-white w-[400px] h-[350px] my-32 mr-100 flex flex-col rounded-xl relative justify-center drop-shadow-lg"
		>
			<form class="flex flex-col">
				<input
					class="px-4 text-gray-400 border py-2 my-2 mx-3"
					type="text"
					placeholder="Email address or phone number"
				/>
				<input
					class="px-4 text-gray-400 border py-2 my-1 mx-3"
					type="text"
					placeholder="Password"
				/>
				<button
					class="bg-blue-600 py-2 my-2 text-white rounded-md px-2 mx-3 cursor-pointer text-md font-bold"
					>Log IN</button
				>
			</form>
			<span class="mx-auto text-blue-600 text-sm my-1">Forgotten password?</span>
			<hr class="my-4 mx-3 opacity-20" />
			<button
				class="bg-green-500 py-2 my-2 text-white rounded-md px-2 mx-3 cursor-pointer text-md font-bold"
				>Create new account</button
			>
			<span class="absolute -bottom-10 text-sm mx-10"
				><span class="cursor-pointer hover:underline font-bold">Create a Page</span> for a celebrity,
				brand or business.</span
			>
		</div>
	</div>
	<div class="bg-white mt-32 text-xs flex-col justify-center flex pt-2 pb-44">
		<span class="mx-40  text-gray-500 my-2"
			>
            <span class=""><pre>    English (UK)  ગુજરાતી  हिन्दी  मराठी  اردو  ਪੰਜਾਬੀ  தமிழ்  తెలుగు  বাংলা  മലയാളം  Español <button class=" border px-2 shadow-inner-xl -py-1 font-bold text-lg">+</button>

            </pre></span>
			<hr class="my-1"/>
			<span
				>Sign Up Log in Messenger Facebook Lite Video Places Games Marketplace Meta Pay Meta Store
				Meta Quest Instagram Threads <br />
				Fundraisers Services Voting Information Centre Privacy Policy Privacy Centre Groups About Createad
				Create Page Developers Careers</span
			>
		</span>
	</div>
</html>
